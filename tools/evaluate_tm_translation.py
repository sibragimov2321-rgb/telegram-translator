"""Compare the existing and conversational Turkmen prompts on curated cases."""
...
import tm_translation as tm  # noqa: E402
...
async def evaluate_case(row: dict, semaphore: asyncio.Semaphore) -> dict:
    async with semaphore:
        ...
        old_target = "Türkmençe (туркменский)" if row["direction"] == "ru_to_tm" else "Русский"
        ...
        started = time.monotonic()
        try:
            new = await bot.translate_turkmen_conversational(row["source"], row["direction"], history)
            new_error = ""
        except Exception as error:
            new = ""
        ...
        source_words = max(1, len(tm.tokens(row["source"])))
        current_entity_errors = tm.missing_or_changed_entities(row["source"], old) if old else ["empty"]
        new_entity_errors = tm.missing_or_changed_entities(row["source"], new) if new else ["empty"]
        new_similarity = similarity(new, row["reference"]) if new else 0
        current_similarity = similarity(old, row["reference"]) if old else 0
        return {
            **row,
            "current": old,
            "new": new,
            ...
            "new_error": new_error,
            "current_reference_similarity": current_similarity,
            "new_reference_similarity": new_similarity,
            "current_entity_errors": current_entity_errors,
            "new_entity_errors": new_entity_errors,
            "meaning_preserved": bool(new and new_similarity >= 0.45 and not new_entity_errors),
            "style_pass": bool(new and not tm.translation_needs_retry(row["source"], new, row["direction"], row.get("style"))),
            "current_length_ratio": round(len(tm.tokens(old)) / source_words, 3) if old else 0,
            "new_length_ratio": round(len(tm.tokens(new)) / source_words, 3) if new else 0,
...
def markdown_report(report: dict) -> str:
    ...
    for index, row in enumerate(report["results"], 1):
        lines.extend([
            ...
            "",
            f"MEANING PRESERVED: {'yes' if row['meaning_preserved'] else 'no'}",
            f"STYLE: {'pass' if row['style_pass'] else 'fail'}",
            f"ENTITY PRESERVATION: {'pass' if not row['new_entity_errors'] else 'fail'}",
            "",
        ])
    return "\n".join(lines)
...
async def run(limit: int, concurrency: int, output_dir: Path, retry_errors: bool = False) -> None:
    rows = json.loads((ROOT / "tests" / "tm_translation_cases.json").read_text(encoding="utf-8"))
    holdout_rows = [row for row in rows if row.get("holdout")]
    if len(holdout_rows) >= limit:
        rows = holdout_rows
    report_path = output_dir / "tm_evaluation.json"
    ...
    (output_dir / "tm_evaluation.json").write_text(
        json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (output_dir / "tm_evaluation.md").write_text(markdown_report(report), encoding="utf-8")
